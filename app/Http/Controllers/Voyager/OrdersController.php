...
class OrdersController extends Controller
{
    ...
    public function sendChargeInvoice(Charge $charge)
    {
        $order = $charge->order;
        $date = $order->created_at->format('M d, Y');
        $mail_invoice_data = [
            'subject' => "New payment request sent for the order  # $order->id",
            'title' => "New payment request sent for the order  # $order->id",
            'opening_message' => " السلام عليكم $order->first_name  $order->last_name  <br>
            طلب سداد جديد ارسل للطلب الذي تم في تاريخ $date <br />
    ...
    public function update_services(Order $order, Request $request)
    {
    ...
    public function update_services_amount(Order $order, Request $request)
    {
        $order->update([
            'total' => $request->total,
    ...
    public function index(Request $request)
    {
        ...
        if (request('key') == 'user_id') {
            $orders = Order::where('user_id', request('s'));
        }
        $orders = $orders->get();
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            //$dataTypeContent = call_user_func([$query->where('status','!=', '0'), $getter]);
